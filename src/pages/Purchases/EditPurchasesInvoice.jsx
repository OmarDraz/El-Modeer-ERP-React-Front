...
const CreatePurchaseInvoice = () => {
    ...
    const rightInputs = [
        {
            ...
            width: '100%',
            placeholder: state.supplier_name,
    ...
    useEffect(() => {
       axiosInstance.get('purchases/update/' + parseInt(invoiceId)).then((res) => {
        let products = []
        res.data.PurchaseInvoice_products.forEach((product) => products.push({
        ...
        res.data.attachment && setAttachments(res.data.attachment)
        setShipping( {
       ...
       setState({...state, add_by: token.id})
    }, []) 
    ...
    function sendData(){
        ...
        )
        {
            ...
            const URL = `http://api.maha-beauty.com/purchases/update/${parseInt(invoiceId)}`
